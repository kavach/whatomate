...
function normalizeSavedPreset(
  ...
  if (saved && (PRESET_VALUES as readonly string[]).includes(saved)) {
    return saved as TimeRangePreset
  }
  return fallback
}

export function useDateRange(options: UseDateRangeOptions = {}) {
  const { defaultPreset = 'all', storageKey } = options
  ...
  const loadSaved = (): { range: TimeRangePreset; customRange: any } => {
    ...
    const savedRange = localStorage.getItem(`${storageKey}_range`)
    ...
    return { range: normalizeSavedPreset(savedRange, defaultPreset), customRange }
  }
  ...
  const dateRange = computed<DateRangeResult>(() => {
    ...
    switch (selectedRange.value) {
      case 'all':
        // Wide bounds for APIs that require from/to; screens that mean “no date filter” should omit params.
